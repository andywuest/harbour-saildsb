...
void DsbParserTests::testParsePlanToJson() {
  ...
  QJsonDocument jsonPlan = dsbParser->parseHtmlToJson(QString(data));
  QCOMPARE(jsonPlan.isObject(), true);
  QCOMPARE(jsonPlan["data"].isArray(), true);
  QCOMPARE(jsonPlan["date"].isString(), true);

  QString planDate = jsonPlan["date"].toString();
  QCOMPARE(planDate, "25.10.2022 Dienstag, Woche A");
  QJsonArray planData = jsonPlan["data"].toArray();
